...
class NutripalCommands extends DrushCommands {
    ...
    public function importfood($filename, $options = [
        ...
        // drush nutripal:importfood modules/custom/nutripal/data/CaloriesPOEI.txt --input-fields=12 --rows-max=10 --rows-offset=10
        $this->logger()->success(t('Import food data...'));
        
        $fdi = new ImportFoodData($this->logger());
        $fdi->parseFoodData($filename, $options['input-head'], $options['input-fields'], $options['input-del'], $options['input-enc'], $options['input-mod'], $options['rows-max'], $options['rows-offset']);
